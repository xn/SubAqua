...
/** War-open test used everywhere in the ash (UTS:626-627, 2237-2239 etc.). */
export function skateWarOpen(): boolean {
  return get("skateParkStatus") === "war" && !get("noncombatQueue").includes("Holey Rollers");
}

/** Lutz = the daily 30-turn Fishy, ice state only (SkateParkRequest.java:35-76;
 * statuseffects.txt:552). */
...
- The run terminates at `questL13Final == "finished"` + penny dump; `postloopCommand` fires from main.ts when every task reports complete.
