...
- **Dropped, priced (table below):** Summon Taffy libram cast + red-taffy underwater throw, pulled-yellow-taffy YR source, Waffle Day / Kokomo dailies, Disco Nap for Marked by the Don.
...
| `sea` maximizer keyword (upstream corral/tame strings)                                                                  | **IN (user decision 2026-08-27; gated on `!hasBreathingEffect()`)** | `Evaluator.java:396-404`: forces the Adventure Underwater + Underwater Familiar booleans, so the maximizer buys the breathing gear itself instead of the engine pinning a slot. Added to the outfit modifier in `customize()` and to the self-dressing Sea helpers (gym, colosseum, skate park) — but ONLY while Driving Waterproofly / Wet Willied is down (`seaKeyword()`, `src/engine/outfit.ts`), since under the effect the keyword would spend a slot on breathing the effect already provides. `dress()`'s last-chance verification stays. |
